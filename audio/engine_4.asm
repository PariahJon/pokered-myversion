...
.startChecks
	...
	call Audio4_ApplyDutyCyclePattern
...
Audio4_note_type:
	and $f0
	cp note_type_cmd
	jp nz, Audio4_toggle_perfect_pitch
	ld a, d
	and $f
	ld b, $0
...
Audio4_toggle_perfect_pitch:
	ld a, d
	cp toggle_perfect_pitch_cmd
	jr nz, Audio4_vibrato
	ld b, 0
	ld hl, wChannelFlags1
	add hl, bc
	ld a, [hl]
...
Audio4_stereo_panning:
	cp stereo_panning_cmd
	jr nz, Audio4_unknownmusic0xef
	call Audio4_GetNextMusicByte
	ld [wStereoPanning], a ; store panning
	jp Audio4_sound_ret

; this appears to never be used
Audio4_unknownmusic0xef:
	cp unknownmusic0xef_cmd
	jr nz, Audio4_duty_cycle_pattern
	call Audio4_GetNextMusicByte
	push bc
...
Audio4_volume:
	cp volume_cmd
	jr nz, Audio4_execute_music
	call Audio4_GetNextMusicByte
	ldh [rAUDVOL], a ; store volume
	jp Audio4_sound_ret

Audio4_execute_music:
...
Audio4_sfx_note:
	cp sfx_note_cmd
	jr nz, Audio4_pitch_sweep
	ld a, c
	cp CHAN4 ; is this a noise or sfx channel?
	jr c, Audio4_pitch_sweep ; no
	ld b, 0
	ld hl, wChannelFlags2
	add hl, bc
	bit BIT_EXECUTE_MUSIC, [hl] ; is execute_music being used?
	jr nz, Audio4_pitch_sweep ; yes
	call Audio4_note_length

; This code seems to do the same thing as what Audio4_ApplyDutyCycleAndSoundLength
; does below.
	ld d, a
	ld b, 0
	ld hl, wChannelDutyCycles
...
.skip
	ld d, a
	push de
	call Audio4_ApplyDutyCycleAndSoundLength
...
Audio4_note:
	ld a, c
	cp CHAN4
	jr nz, Audio4_note_length ; if not noise channel
	ld a, d
	and $f0
	cp drum_note_cmd
	jr z, .drum_note
	jr nc, Audio4_note_length
...
.drum_note
	...
	push af
	push bc
	call Audio4_GetNextMusicByte ; get drum_note instrument
.playDnote
	ld d, a
	ld a, [wDisableChannelOutputWhenSfxEnds]
	and a
...
.skip
	...
	add hl, bc
	bit BIT_EXECUTE_MUSIC, [hl]
	jr nz, Audio4_note_pitch
	ld hl, wChannelFlags1
	add hl, bc
	bit BIT_NOISE_OR_SFX, [hl]
	jr z, Audio4_note_pitch
	pop hl
	ret
...
.notRest
	...
	bit BIT_PITCH_SLIDE_ON, [hl]
	jr z, .skipPitchSlide
	call Audio4_InitPitchSlideVars
.skipPitchSlide
	push de
	ld a, c
	cp CHAN5
...
Audio4_ApplyDutyCycleAndSoundLength:
	ld b, 0
...
Audio4_ApplyWavePatternAndFrequency:
	ld a, c
	cp CHAN3
	jr z, .channel3
	cp CHAN7
